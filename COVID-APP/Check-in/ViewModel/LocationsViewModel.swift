    ...
    func fetchData() {
        ...
        db.collection("businessSign").addSnapshotListener{(QuerySnapshot, error) in
            ...
            tempArray = documents.map{ (QueryDocumentSnapshot) -> CovidSign in
                ...
                return CovidSign(id: id, businessID: businessID, name: name, longitude: longitude, latitude: latitude, qrCode: qrCode)
        ...
        db.collection("businessSign").whereField("active", isEqualTo: true).limit(to: 10).getDocuments() { (querySnapshot, err) in
            if let err = err {
                ...
                for document in querySnapshot!.documents {
                    ...
                    let busSignLocation = CovidSign(id: fsSignID, businessID: fsBusinessID, name: fsName, longitude: fsLongitude, latitude: fsLatitude, qrCode: fsQrCode)

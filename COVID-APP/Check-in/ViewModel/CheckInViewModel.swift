...
class CheckInVeiwModel: ObservableObject
{
    ...
    init() {
        ...
        busSignLocation = CovidSign(id: "", businessID: ref, name: "", location: GeoPoint(latitude: 0, longitude: 0), qrCode: "")
        ...
        db.collection("businessSign").whereField("qrCode", isEqualTo: url).limit(to: 1).getDocuments() { (querySnapshot, err) in
            if let err = err {
                ...
                for document in querySnapshot!.documents {
                    ...
                    let fsBusinessID = data["businessID"] as? DocumentReference ?? nil
                    let fsQrCode = data["qrCode"] as? String ?? ""
                    let fsBusinessSignID = data["businessSignID"] as? String ?? ""
                    let fsLocation = data["location"] as? GeoPoint ?? GeoPoint(latitude: 0, longitude: 0)
                    let fsName = data["name"] as? String ?? ""
                    print("bid")
                    print(fsLocation)
                    
                    self.busSignLocation = CovidSign(id: fsBusinessSignID, businessID: fsBusinessID!, name: fsName, location: fsLocation, qrCode: fsQrCode)
